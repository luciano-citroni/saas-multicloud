...
import { DescribeCacheClustersCommand, ListTagsForResourceCommand, type CacheCluster } from '@aws-sdk/client-elasticache';
        ...
        for (const cluster of CacheClusters) {
            ...
            if (cluster.ARN) {
                try {
                    ...
                    tags =
                        TagList?.reduce<Record<string, string>>((acc, t) => {
...
function mapDbCluster(cluster: AwsElastiCacheCluster) {
    return {
        ...
        vpc: cluster.vpc ? { id: cluster.vpc.id, awsVpcId: cluster.vpc.awsVpcId, cidrBlock: cluster.vpc.cidrBlock, state: cluster.vpc.state } : null,
    };
}
