...
export class Ec2Controller {
    ...
    // =========================================================================
    // Instâncias EC2 - Banco de Dados
    // =========================================================================

    @Get('accounts/:cloudAccountId/instances')
    @ApiOperation({
        summary: 'Listar instâncias EC2 do banco de dados',
        description: 'Retorna as instâncias EC2 sincronizadas e armazenadas no banco de dados.',
    })
    @ApiParam({ name: 'cloudAccountId', type: 'string', format: 'uuid' })
    @ApiResponse({
        status: 200,
        description: 'Lista de instâncias EC2 do banco de dados',
    ...
    @ApiOperation({
        summary: 'Listar instâncias EC2 de uma VPC específica',
        description: 'Retorna as instâncias EC2 de uma VPC específica armazenadas no banco de dados.',
    })
    @ApiParam({ name: 'cloudAccountId', type: 'string', format: 'uuid' })
    @ApiParam({ name: 'vpcId', type: 'string', format: 'uuid', description: 'UUID da VPC no banco de dados' })
    @ApiResponse({
        status: 200,
        description: 'Lista de instâncias EC2 da VPC',
        type: [Ec2InstanceResponseDto],
    })
    ...
    @ApiOperation({
        summary: 'Buscar instância EC2 por ID',
        description: 'Retorna os detalhes de uma instância EC2 específica.',
    })
    @ApiParam({ name: 'cloudAccountId', type: 'string', format: 'uuid' })
    ...
    @HttpCode(200)
    @ApiOperation({
        summary: 'Sincronizar instâncias EC2 da AWS',
        description: 'Busca as instâncias EC2 da AWS e armazena/atualiza no banco de dados. Use ?vpcId= para sincronizar apenas uma VPC específica.',
    })
    @ApiParam({ name: 'cloudAccountId', type: 'string', format: 'uuid' })
    @ApiQuery({ name: 'vpcId', required: false, description: 'Filtrar sync por VPC ID (UUID do banco de dados)', type: 'string' })
    @ApiResponse({
        status: 200,
        description: 'Instâncias EC2 sincronizadas com sucesso',
