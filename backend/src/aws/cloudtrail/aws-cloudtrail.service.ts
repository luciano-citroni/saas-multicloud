...
import { DescribeTrailsCommand, GetTrailStatusCommand, ListTagsCommand, type Trail } from '@aws-sdk/client-cloudtrail';
...
export class AwsCloudTrailService {
    ...
    async syncTrailsFromAws(cloudAccountId: string, organizationId: string) {
        ...
        for (const trail of trailList) {
            ...
            const vpc = trailData.awsVpcId ? await this.vpcRepository.findOne({ where: { cloudAccountId, awsVpcId: trailData.awsVpcId } }) : null;
...
function mapDbTrail(trail: AwsCloudTrailTrail) {
    return {
        ...
        vpc: trail.vpc ? { id: trail.vpc.id, awsVpcId: trail.vpc.awsVpcId, cidrBlock: trail.vpc.cidrBlock, state: trail.vpc.state } : null,
    };
}
