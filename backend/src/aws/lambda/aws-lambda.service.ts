...
import { ListFunctionsCommand, GetFunctionCommand, ListTagsCommand, type FunctionConfiguration } from '@aws-sdk/client-lambda';
...
export class AwsLambdaService {
    ...
    async syncFunctionsFromAws(cloudAccountId: string, organizationId: string) {
        ...
        for (const fn of Functions) {
            ...
            const vpc = fnData.awsVpcId ? await this.vpcRepository.findOne({ where: { cloudAccountId, awsVpcId: fnData.awsVpcId } }) : null;

            if (fnData.awsVpcId && !vpc) {
                this.logger.debug(`VPC ${fnData.awsVpcId} não encontrada no banco. Sincronize as VPCs primeiro.`);
            }

            const iamRole = fnData.roleArn ? await this.iamRoleRepository.findOne({ where: { cloudAccountId, roleArn: fnData.roleArn } }) : null;
...
function mapDbFunction(fn: AwsLambdaFunction) {
    return {
        ...
        vpc: fn.vpc ? { id: fn.vpc.id, awsVpcId: fn.vpc.awsVpcId, cidrBlock: fn.vpc.cidrBlock, state: fn.vpc.state } : null,

        iamRole: fn.iamRole ? { id: fn.iamRole.id, roleArn: fn.iamRole.roleArn, roleName: fn.iamRole.roleName } : null,
    };
}
