...
export class RolesGuard implements CanActivate {
    ...
    canActivate(context: ExecutionContext): boolean {
        const requiredRoles = this.reflector.getAllAndOverride<OrgRole[]>(ROLES_KEY, [context.getHandler(), context.getClass()]);
        ...
        if (!membership) {
            throw new ForbiddenException('No organization membership found. Ensure TenantGuard runs before RolesGuard.');
        }
        ...
        if (userLevel < minimumRequired) {
            throw new ForbiddenException(`Insufficient permissions. Required: ${requiredRoles.join(' or ')}. Your role: ${userRole}.`);
        }
