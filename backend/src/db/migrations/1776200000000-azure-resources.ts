...
export class AzureResources1776200000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const createTable = async (table: string, extraCols = ''): Promise<void> => {
            ...
            `);
            await queryRunner.query(`
                CREATE INDEX "IDX_azure_${table}_cloud_account_id" ON "azure"."${table}" ("cloud_account_id")
            `);
        };
        ...
        // ── Compute ────────────────────────────────────────────────────────────
        await createTable('virtual_machines', `
            "vm_size"            varchar(100),
            "os_type"            varchar(20),
            "provisioning_state" varchar(50),
        `);

        await createTable('vmss', `
            "sku"                varchar(100),
            "provisioning_state" varchar(50),
        `);

        await createTable('aks_clusters', `
            "kubernetes_version" varchar(30),
            "provisioning_state" varchar(50),
        `);

        await createTable('disks', `
            "disk_size_gb" int,
        ...
        // ── Web ────────────────────────────────────────────────────────────────
        await createTable('web_apps', `
            "kind"  varchar(50),
            "state" varchar(20),
        `);

        await createTable('app_service_plans', `
            "sku"  varchar(50),
            "kind" varchar(50),
        `);
        ...
        await createTable('network_interfaces', `
            "private_ip_address" varchar(50),
            "provisioning_state" varchar(50),
        `);

        await createTable('public_ips', `
        ...
        await createTable('application_gateways', `
            "sku"                varchar(50),
            "provisioning_state" varchar(50),
        `);

        // ── Storage ────────────────────────────────────────────────────────────
        await createTable('storage_accounts', `
            "sku"  varchar(50),
            "kind" varchar(50),
        `);

        // ── Databases ──────────────────────────────────────────────────────────
        await createTable('sql_servers', `
            "fully_qualified_domain_name" varchar(255),
            "administrator_login"         varchar(100),
        `);

        await createTable('sql_databases', `
            "sku"    varchar(100),
            "status" varchar(50),
        `);

        await createTable('postgres_servers', `
            "administrator_login" varchar(100),
            "version"             varchar(20),
        `);

        await createTable('cosmos_db_accounts', `
            "database_account_offer_type" varchar(50),
            "document_endpoint"           varchar(255),
        `);

        // ── Security ───────────────────────────────────────────────────────────
        await createTable('key_vaults', `
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        const tables = [
            'subscriptions', 'resource_groups', 'virtual_machines', 'vmss', 'aks_clusters',
            'disks', 'web_apps', 'app_service_plans', 'virtual_networks', 'network_interfaces',
            'public_ips', 'nsgs', 'load_balancers', 'application_gateways', 'storage_accounts',
            'sql_servers', 'sql_databases', 'postgres_servers', 'cosmos_db_accounts',
            'key_vaults', 'recovery_vaults',
        ];
