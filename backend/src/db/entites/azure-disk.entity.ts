...
@Entity('disks', { schema: 'azure' })
@Unique('UQ_azure_disks_account_azure_id', ['cloudAccountId', 'azureId'])
export class AzureDisk {
    ...
    @Column({ type: 'int', name: 'disk_size_gb', nullable: true })
    diskSizeGb!: number | null;

    @Column({ type: 'varchar', length: 50, name: 'disk_state', nullable: true })
    diskState!: string | null;

    @Column({ type: 'varchar', length: 50, name: 'sku', nullable: true })
    sku!: string | null;
