...
type Params = { params: Promise<{ cloudAccountId: string; anomalyId: string }> };
...
export async function PATCH(request: Request, { params }: Params) {
    ...
    const { cloudAccountId, anomalyId } = await params;
    ...
    let response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/anomalies/${anomalyId}/acknowledge`, {
        method: 'PATCH',
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/anomalies/${anomalyId}/acknowledge`, {
            method: 'PATCH',
