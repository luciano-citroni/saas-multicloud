...
export async function GET(request: Request, { params }: Params) {
    ...
    if (months) backendParams.set('months', months);

    const backendUrl = `/api/finops/accounts/${cloudAccountId}/forecast/next-months${backendParams.toString() ? `?${backendParams.toString()}` : ''}`;
