...
export async function POST(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/billing/compute`, {
        method: 'POST',
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/billing/compute`, {
            method: 'POST',
