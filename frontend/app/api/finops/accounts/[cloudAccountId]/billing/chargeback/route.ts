...
export async function GET(request: Request, { params }: Params) {
    ...
    if (tagKey) backendParams.set('tagKey', tagKey);
    if (startDate) backendParams.set('startDate', startDate);
    if (endDate) backendParams.set('endDate', endDate);

    const backendUrl = `/api/finops/accounts/${cloudAccountId}/billing/chargeback${backendParams.toString() ? `?${backendParams.toString()}` : ''}`;
