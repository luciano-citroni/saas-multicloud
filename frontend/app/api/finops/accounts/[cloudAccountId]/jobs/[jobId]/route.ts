...
type Params = { params: Promise<{ cloudAccountId: string; jobId: string }> };
...
export async function GET(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/jobs/${jobId}`, {
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/jobs/${jobId}`, {
