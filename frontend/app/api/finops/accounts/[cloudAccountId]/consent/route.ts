...
export async function GET(request: Request, { params }: Params) {
    ...
    if (!cloudProvider) return NextResponse.json({ message: 'cloudProvider é obrigatório' }, { status: 400 });

    const resolved = await resolveAccessToken();
    if (!resolved.accessToken) return buildUnauthorizedResponse();

    let response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/consent?cloudProvider=${cloudProvider}`, {
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/consent?cloudProvider=${cloudProvider}`, {
...
export async function POST(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/consent`, {
        method: 'POST',
        headers: { ...withAuthHeaders(resolved.accessToken, organizationId), 'Content-Type': 'application/json' },
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        if (!refreshResult) return buildUnauthorizedResponse();
        rotatedTokens = refreshResult;
        response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/consent`, {
            method: 'POST',
            headers: { ...withAuthHeaders(refreshResult.accessToken, organizationId), 'Content-Type': 'application/json' },
            body: JSON.stringify(body),
        });
    }
...
export async function DELETE(request: Request, { params }: Params) {
    ...
    const nextResponse = response.status === 204
        ...
        : NextResponse.json(await parseJsonSafe<unknown>(response) ?? {}, { status: response.status });
