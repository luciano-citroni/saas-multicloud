...
export async function GET(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/cost-by-service${queryString}`, {
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/finops/accounts/${cloudAccountId}/cost-by-service${queryString}`, {
