...
type Params = {
    params: Promise<{ cloudAccountId: string }>;
};
...
export async function POST(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/governance/accounts/${cloudAccountId}/scan`, {
        method: 'POST',
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/governance/accounts/${cloudAccountId}/scan`, {
            method: 'POST',
    ...
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao iniciar scan de governança' }, { status: response.status });
