...
export async function GET(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/gcp/assessment/accounts/${cloudAccountId}/assessment-jobs/${jobId}`, {
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/gcp/assessment/accounts/${cloudAccountId}/assessment-jobs/${jobId}`, {
            headers: withAuthHeaders(refreshResult.accessToken, organizationId),
        });
    }

    const payload = await parseJsonSafe<unknown>(response);
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao consultar job GCP' }, { status: response.status });
