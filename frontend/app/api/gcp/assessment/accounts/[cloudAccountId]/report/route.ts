...
export async function POST(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/gcp/assessment/accounts/${cloudAccountId}/report`, {
        method: 'POST',
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/gcp/assessment/accounts/${cloudAccountId}/report`, {
            method: 'POST',
    ...
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao gerar relatório Excel GCP' }, { status: response.status });
