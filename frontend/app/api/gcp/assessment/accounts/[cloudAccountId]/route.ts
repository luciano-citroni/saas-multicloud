...
export async function POST(request: Request, { params }: Params) {
    ...
    let response = await backendFetch(`/api/gcp/assessment/accounts/${cloudAccountId}`, {
        method: 'POST',
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/gcp/assessment/accounts/${cloudAccountId}`, {
            method: 'POST',
    ...
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao enfileirar assessment GCP' }, { status: response.status });
