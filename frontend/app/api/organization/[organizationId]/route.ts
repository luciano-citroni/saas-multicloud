...
export async function GET(_: Request, context: RouteContext) {
    ...
    let response = await backendFetch(`/api/organization/${encodeURIComponent(organizationId)}`, {
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/organization/${encodeURIComponent(organizationId)}`, {
...
export async function PATCH(request: Request, context: RouteContext) {
    ...
    const payload = await parseJsonSafe<unknown>(response);
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao atualizar organização' }, { status: response.status });
...
export async function DELETE(request: Request, context: RouteContext) {
    ...
    const payload = await parseJsonSafe<unknown>(response);
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao excluir organização' }, { status: response.status });
