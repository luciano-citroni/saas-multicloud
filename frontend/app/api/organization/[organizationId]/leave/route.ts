...
export async function DELETE(request: Request, context: RouteContext) {
    ...
    let response = await backendFetch(`/api/organization/${encodeURIComponent(organizationId)}/leave`, {
        method: 'DELETE',
    ...
    if (response.status === 401 && resolved.refreshToken) {
        ...
        response = await backendFetch(`/api/organization/${encodeURIComponent(organizationId)}/leave`, {
            method: 'DELETE',
    ...
    const nextResponse = NextResponse.json(payload ?? { message: 'Erro ao sair da organização' }, { status: response.status });
